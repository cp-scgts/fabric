...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/scgts.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``Scgts`` & ``Org2``) each managing and maintaining two peer nodes.
...
# (``peer0.scgts.com`` & ``peer0.org2.example.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for ScgtsMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/ScgtsMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ScgtsMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ScgtsMSP..."
    exit 1
  fi
